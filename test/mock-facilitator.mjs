...
//
// EXTRACTED from x402-settlement.test.mjs on 2026-09-01, behaviour unchanged
// except for the two additions named below, so that x402-solana.test.mjs can
// drive the same upstream. Two suites asserting against two hand-written mocks
// would be two different definitions of what the facilitator does, and the
// strictness below is the whole reason these tests mean anything — duplicating
// it is how it rots in one copy.
//
...
//                    second rail: the Worker reads `extra.feePayer` out of it for
//                    every Solana accepts entry it publishes, and the two
//                    protocol versions carry DIFFERENT feePayers here on purpose,
//                    because they do upstream (CDP draws them from a pool) and a
//                    per-version cache that mixed them up would otherwise pass.
//
...
// signature from what it is handed. So every hit is shape-checked against its own
// declared version and a mismatch answers 400, which surfaces as an unverified
// serve and fails whatever test made the call. Drift is meant to be loud.
// (GET /supported carries no payment and is not shape-checked.)

import http from 'node:http';

// A payer address and a settlement hash the mock hands back, so the ledger
// assertions can prove the values came from the FACILITATOR rather than from the
...
// consecutive reads actually returned for the same version+network row
// (2026-08-31). A Worker that fetched once and reused the answer for both
// protocol versions would publish a v2 entry naming the v1 fee payer, which is a
// payment nobody pays the fee for — and it would be invisible against a fixture
// that used one address twice.
...
export const SOLANA_SIGNATURE =
  '5UfDuX1v2mUUEPvGXQU8QU8m5N2xxVBiwmSSTSKEDBmT7ZgPLjKmxjRTkBhmDGtoUwrCzKrmwVSzMSFvMiSbaHmz';
...
export const supportedFixture = () => ({
  kinds: [
    ...
    // does advertise `upto` alongside `exact` on Solana v2.
    {
      ...
      extra: { feePayer: 'NOTtheFeePayerForExact11111111111111111111' },
    },
  ],
});
 ...
 * it is not enough that a v1 body has `maxAmountRequired`, it must ALSO not have
 * `amount`, because the failure this guards against is a half-migrated envelope
 * carrying both and being waved through by a lenient reader.
 */
export function shapeProblem(body) {
  ...
  if (version === 1) {
    return (
      ...
      // change turns on: the requirements must be for the chain the payload says
      // it paid on. A Solana payload checked against the Base entry is a
      // perfectly good payment the facilitator cannot recover. A v1 payload that
      // names no network at all keeps the pre-Solana behaviour and is not
      // second-guessed here — the Worker takes the first entry, which is Base.
      (payload.network !== undefined && payload.network !== req.network
  ...
  return (
    ...
    // The signature was made over `accepted`, so a requirements object that does
    // not match it is a payment the facilitator cannot recover. Compared
    // key-order-independently, the way x402's own server does it — a client that
    // re-serialises our entry has not changed the offer. (This subsumes the
    // network check the v1 branch has to make explicitly.)
  ...
  const server = http.createServer((req, res) => {
    ...
    req.on('end', async () => {
      ...
      if (endpoint === 'supported') {
        ...
        const supportedDelay = state.delayMs.supported || 0;
        if (supportedDelay) await new Promise((r) => setTimeout(r, supportedDelay));
        const answer = state.supported;
      ...
      // enforced, so a test can assert on it directly as well as through the 400.
