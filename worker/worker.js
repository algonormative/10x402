  ...
  const res = json({
    ...
    endpoints: [
      ...
      ...ENDPOINTS.map((e) => ({
        ...
        sample: e.sample,
        // What a SERVED report looks like, free, before any payment: the
        // published sample input run through the real engine, as a static
        // file on the read surface. Generated by build.mjs from the same
        // runSample() the docs use, so it cannot drift from a paid answer.
        sample_report: `${SITE_BASE}/samples/${e.id}.json`,
      })),
    ],
...
async function handlePaid(request, env, ctx, endpoint) {
  ...
  // THE RECEIPT SAYS WHAT ELSE EXISTS. A single-check answer used to end the
  // conversation: one verdict, no indication that the other checks even
  // existed. The funnel showed every sale to date is a single check at the
  // bottom of the price sheet — including one buyer running four in a burst,
  // which is the designed CI pattern — while the full report has never sold
  // once. Whatever else is true, no buyer has ever been TOLD, at the moment
  // of purchase, what the fuller answer costs or looks like. This is that
  // sentence: the paired route, its price, and a free sample of its output,
  // stated as fact on the receipt. Decoration only — it changes no verdict,
  // no grade, and nothing recordLintSafely stores.
  if (endpoint.single) {
    const full = ENDPOINTS_BY_ID.get(endpoint.pairedWith);
    if (full) {
      report.full_report = {
        path: full.path,
        price: priceLabel(full.price_usd),
        checks_total: CHECKS.length,
        sample: `${SITE_BASE}/samples/${full.id}.json`,
        note:
          `this call answered exactly one check; ${full.path} runs all ${CHECKS.length} ` +
          'that apply, against the same input',
      };
    }
  }
